...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = ""   # ← never hardcode a real token here; use the BOT_TOKEN env var
TOKEN = (
...
try:
    # No hardcoded fallback owner ID — leave OWNER_ID env var unset and the
    # first person to send /start automatically becomes the panel owner.
...
BRAND       = os.environ.get("BOT_BRAND", "My Hosting Bot")
BRAND_VER   = "v2.1"
BRAND_TAG   = f"{BRAND} {BRAND_VER}"
SUPPORT_USR = os.environ.get("SUPPORT_USERNAME", "@your_support_here")
UPDATE_CH   = os.environ.get("UPDATE_CHANNEL_URL", "https://t.me/your_channel_here")
...
PAYMENT_METHODS: Dict[str, Dict[str, Any]] = {
    "bkash":   {"name": "bKash",       "number": os.environ.get("PAY_BKASH_NUMBER", "Not configured"),   "type": "Send Money",        "tag": "[B]"},
    "nagad":   {"name": "Nagad",       "number": os.environ.get("PAY_NAGAD_NUMBER", "Not configured"),   "type": "Send Money",        "tag": "[N]"},
    "rocket":  {"name": "Rocket",      "number": os.environ.get("PAY_ROCKET_NUMBER", "Not configured"),  "type": "Send Money",        "tag": "[R]"},
    "upay":    {"name": "Upay",        "number": os.environ.get("PAY_UPAY_NUMBER", "Not configured"),    "type": "Send Money",        "tag": "[U]"},
    "binance": {"name": "Binance Pay", "number": os.environ.get("PAY_BINANCE_ID", "Not configured"),     "type": "USDT (BEP20/TRC20)","tag": "[BP]"},
    "bank":    {"name": "Bank",        "number": os.environ.get("PAY_BANK_INFO", "Contact admin"),       "type": "Bank Transfer",     "tag": "[BK]"},
}
...
def get_authorized_users() -> set:
    """Usernames (lowercase, no @) explicitly granted access via .auth."""
    return set(get_setting("authorized_users", []) or [])


def is_user_authorized(call_or_msg: Any) -> bool:
    """Gate for whether this person may use the bot at all.

    Owner/admins always pass. Everyone else must have been added with
    .auth @username. Before the panel has an owner yet, we allow the
    very first /start through so the original auto-claim-ownership flow
    still works.
    """
    ...
    if is_admin(uid):
        return True
    uname = (getattr(call_or_msg.from_user, "username", None) or "").lower()
    return bool(uname) and uname in get_authorized_users()


def _handle_auth_commands(m: "types.Message", uid: int, text: str) -> bool:
    """.auth @username / .dauth @username — owner/admin only.
    Returns True if the message was an auth command (handled)."""
    low = text.lower()
    if not (low.startswith(".auth") or low.startswith(".dauth")):
        return False
    if not is_admin(uid):
        bot.reply_to(m, f"{G['no']} {sc('Owner / admin only')}.")
        return True
    is_deauth = low.startswith(".dauth")
    parts = text.split(maxsplit=1)
    target = parts[1].strip().lstrip("@").lower() if len(parts) == 2 else ""
    if not target:
        usage = ".dauth @username" if is_deauth else ".auth @username"
        bot.reply_to(m, f"{G['warn']} {sc('Usage')}: <code>{usage}</code>", parse_mode="HTML")
        return True
    auth_list = get_authorized_users()
    if is_deauth:
        auth_list.discard(target)
        set_setting("authorized_users", sorted(auth_list))
        audit(uid, "deauth_user", target)
        bot.reply_to(m, f"{G['ok']} <code>@{esc(target)}</code> {sc('permission removed')}.", parse_mode="HTML")
    else:
        auth_list.add(target)
        set_setting("authorized_users", sorted(auth_list))
        audit(uid, "auth_user", target)
        bot.reply_to(m, f"{G['ok']} <code>@{esc(target)}</code> {sc('granted permission to use the bot')}.", parse_mode="HTML")
    return True
...
def banned_block(call_or_msg: Any) -> bool:
    if not is_user_authorized(call_or_msg):
        try:
            chat = call_or_msg.message.chat.id if hasattr(call_or_msg, "message") else call_or_msg.chat.id
            bot.send_message(
                chat,
                f"<b>{G['no']} {sc('You are not authorized to use this bot')}</b>\n"
                f"Ask an admin to run <code>.auth @yourusername</code>, or contact {SUPPORT_USR}.",
                parse_mode="HTML",
            )
        except Exception:
            pass
...
def on_text(m: types.Message) -> None:
    ...
    if _handle_auth_commands(m, uid, text):
        return
    if text.startswith("/"):
